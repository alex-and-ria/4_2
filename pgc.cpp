...
class intf{
	public:
		target tgp;
		bool ret=0;
		intf(int argc, char** argv){
			if (argc > 5){
				PrintUsage (); ret=1;
			}
			else tgp=target(argc,argv);
		}
		void PrintUsage (){
...
class cw: public target{
	public:
		cw(target tg){
			...
			nMessageSize = tg.nMessageSize;	//The message size that the ICMP echo request should carry with it
			nCount = tg.nCount;	//Number of times the request has to be send
			szRemoteIP=tg.szRemoteIP;
			szRemoteHost=tg.szRemoteHost;
			SendBuffer=tg.SendBuffer;
		}
		bool Initialize (){
		...
		int png(){
			...
			while (nPacketsSent < nCount){
				...
				nPacketsSent++;
				auto sckerr=SOCKET_ERROR;//gserrv();
				if (nResult == sckerr){
